...
> ⚡ **Cách nhanh (khuyến nghị):** Toàn bộ Bước B→G bên dưới đã được đóng gói thành script bash trong thư mục [scripts/](scripts/). Cấu trúc tách bạch để dễ bảo trì:
>
> | File | Vai trò | Bạn có sửa? |
> |------|---------|-------------|
> | [scripts/config.sh](scripts/config.sh) | Biến môi trường (org id, billing, bucket, group) | ✅ điền giá trị |
> | [scripts/roles.sh](scripts/roles.sh) | **Bảng phân quyền** — thêm/bớt role chỉ tại đây | ✅ khi cần đổi role |
> | [scripts/lib.sh](scripts/lib.sh) | Hàm trợ giúp (gói lệnh `gcloud`) | ❌ không cần |
> | [scripts/01-bootstrap.sh](scripts/01-bootstrap.sh) · [02](scripts/02-post-org-roles.sh) · [03](scripts/03-runtime-sa.sh) | Script chạy chính | ❌ chỉ chạy |
>
> ```bash
> # 1) Sửa scripts/config.sh (giá trị môi trường) — và scripts/roles.sh nếu muốn đổi role
> gcloud auth login && gcloud auth application-default login   # Bước A
> ./scripts/01-bootstrap.sh                                    # Bước B→G
> # …sửa backend.tf (Bước H) + kiểm tra tfvars (Bước I), rồi: cd org && terraform init && terraform apply
> ./scripts/02-post-org-roles.sh                               # Bước J (sau apply org)
> ./scripts/03-runtime-sa.sh                                   # Phase B (runtime SA, tùy chọn --astro/--tools)
> ```
>
> 👉 **Muốn thêm 1 role cho SA?** Chỉ cần thêm 1 dòng vào bảng tương ứng trong [scripts/roles.sh](scripts/roles.sh) (ví dụ bảng `[1]` cho role cấp org, `[6]` cho role project-level) — không phải đụng vào script chạy.
>
> Phần còn lại của mục này mô tả từng lệnh để bạn hiểu/tùy biến hoặc chạy thủ công.

> ⚠️ **Chạy tất cả các bước dưới đây trong CÙNG một cửa sổ terminal** — các biến `export` (như `$SA_ORG`, `$STATE_BUCKET`) cần giữ nguyên giá trị xuyên suốt. Nếu mở terminal mới, hãy chạy lại **Bước 0**.

**Bước 0 — Biến môi trường (điều chỉnh theo giá trị thực tế của bạn)**
...
**Bước A — Đăng nhập với tài khoản cá nhân có quyền Organization Admin**
...
**Bước B — Tạo Seed Project & bật API**
...
**Bước C — Tạo GCS State Bucket với Object Versioning**
...
**Bước D — Tạo 5 TF Runner SA (mỗi stack một SA)**

```bash
gcloud iam service-accounts create sa-tf-org-001  --project=$SEED_PROJECT --display-name="TF Runner for org stack"
gcloud iam service-accounts create sa-tf-conn-001 --project=$SEED_PROJECT --display-name="TF Runner for connectivity stack"
gcloud iam service-accounts create sa-tf-sec-001  --project=$SEED_PROJECT --display-name="TF Runner for security stack"
gcloud iam service-accounts create sa-tf-wl-001   --project=$SEED_PROJECT --display-name="TF Runner for workload stack"
gcloud iam service-accounts create sa-tf-mgmt-001 --project=$SEED_PROJECT --display-name="TF Runner for management stack"
...
**Bước E — Gán role TỐI THIỂU cho mỗi TF Runner SA (đúng phạm vi stack đó dùng)**

```bash
# E1. sa-tf-org-001 — tạo Folder/Project, Org Policies, link Billing
gcloud organizations add-iam-policy-binding $ORG_ID --member="serviceAccount:$SA_ORG" --role="roles/resourcemanager.organizationAdmin" --condition=None
gcloud organizations add-iam-policy-binding $ORG_ID --member="serviceAccount:$SA_ORG" --role="roles/resourcemanager.projectCreator" --condition=None
gcloud organizations add-iam-policy-binding $ORG_ID --member="serviceAccount:$SA_ORG" --role="roles/serviceusage.serviceUsageAdmin" --condition=None
gcloud organizations add-iam-policy-binding $ORG_ID --member="serviceAccount:$SA_ORG" --role="roles/orgpolicy.policyAdmin" --condition=None
gcloud billing accounts add-iam-policy-binding $BILLING_ACCOUNT_1 --member="serviceAccount:$SA_ORG" --role="roles/billing.user"
gcloud billing accounts add-iam-policy-binding $BILLING_ACCOUNT_2 --member="serviceAccount:$SA_ORG" --role="roles/billing.user"

# E2. sa-tf-conn-001 — VPC/Subnet/Firewall/Router/NAT/VPN/DNS + Shared VPC
gcloud organizations add-iam-policy-binding $ORG_ID --member="serviceAccount:$SA_CONN" --role="roles/compute.xpnAdmin" --condition=None
# Quyền network/DNS gán ở cấp Organization cho gọn (stack connectivity động vào fldr-platform + fldr-workload)
gcloud organizations add-iam-policy-binding $ORG_ID --member="serviceAccount:$SA_CONN" --role="roles/compute.networkAdmin" --condition=None
gcloud organizations add-iam-policy-binding $ORG_ID --member="serviceAccount:$SA_CONN" --role="roles/dns.admin" --condition=None

# E3. sa-tf-sec-001 — Org IAM cho admin_principals, Org Firewall Policies, Log View IAM
gcloud organizations add-iam-policy-binding $ORG_ID --member="serviceAccount:$SA_SEC" --role="roles/resourcemanager.organizationAdmin" --condition=None
gcloud organizations add-iam-policy-binding $ORG_ID --member="serviceAccount:$SA_SEC" --role="roles/compute.orgFirewallPolicyAdmin" --condition=None
# Set IAM của log view nằm trong management project — chờ project tồn tại nên gán SAU khi apply stack org (xem Bước J)
...
# (chờ project tồn tại nên gán SAU khi apply stack org — xem Bước J)

# E5. sa-tf-mgmt-001 — Log Sinks org+folder, Monitoring, Budget
gcloud organizations add-iam-policy-binding $ORG_ID --member="serviceAccount:$SA_MGMT" --role="roles/logging.admin" --condition=None
# Quyền monitoring/logging trên management project và quyền billing budget — gán SAU khi apply stack org (Bước J)
```

**Bước F — Cấp Token Creator cho từng team trên ĐÚNG SA của team mình**

```bash
gcloud iam service-accounts add-iam-policy-binding $SA_ORG  --project=$SEED_PROJECT --member="$GRP_FOUNDATION" --role="roles/iam.serviceAccountTokenCreator"
gcloud iam service-accounts add-iam-policy-binding $SA_CONN --project=$SEED_PROJECT --member="$GRP_NETWORK"    --role="roles/iam.serviceAccountTokenCreator"
gcloud iam service-accounts add-iam-policy-binding $SA_SEC  --project=$SEED_PROJECT --member="$GRP_SECURITY"   --role="roles/iam.serviceAccountTokenCreator"
gcloud iam service-accounts add-iam-policy-binding $SA_WL   --project=$SEED_PROJECT --member="$GRP_APP"        --role="roles/iam.serviceAccountTokenCreator"
gcloud iam service-accounts add-iam-policy-binding $SA_MGMT --project=$SEED_PROJECT --member="$GRP_SRE"        --role="roles/iam.serviceAccountTokenCreator"
```

**Bước G — Cô lập GCS state: mỗi SA chỉ truy cập prefix của mình + đọc upstream**

```bash
# G1. Tất cả 5 SA cần "thấy" được bucket để terraform init nhận diện backend
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_ORG"  --role="roles/storage.legacyBucketReader"
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_CONN" --role="roles/storage.legacyBucketReader"
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_SEC"  --role="roles/storage.legacyBucketReader"
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_WL"   --role="roles/storage.legacyBucketReader"
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_MGMT" --role="roles/storage.legacyBucketReader"

# G2. Mỗi SA: objectAdmin chỉ trên prefix của stack mình (đọc + ghi state)
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_ORG"  --role="roles/storage.objectAdmin" --condition="expression=resource.name.startsWith(\"projects/_/buckets/${STATE_BUCKET}/objects/terraform/org/\"),title=own-state-org"
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_CONN" --role="roles/storage.objectAdmin" --condition="expression=resource.name.startsWith(\"projects/_/buckets/${STATE_BUCKET}/objects/terraform/connectivity/\"),title=own-state-connectivity"
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_SEC"  --role="roles/storage.objectAdmin" --condition="expression=resource.name.startsWith(\"projects/_/buckets/${STATE_BUCKET}/objects/terraform/security/\"),title=own-state-security"
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_WL"   --role="roles/storage.objectAdmin" --condition="expression=resource.name.startsWith(\"projects/_/buckets/${STATE_BUCKET}/objects/terraform/workload/\"),title=own-state-workload"
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_MGMT" --role="roles/storage.objectAdmin" --condition="expression=resource.name.startsWith(\"projects/_/buckets/${STATE_BUCKET}/objects/terraform/management/\"),title=own-state-management"

# G3. Downstream stacks: objectViewer trên prefix của upstream (chỉ đọc remote state)
#   connectivity → đọc org
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_CONN" --role="roles/storage.objectViewer" --condition="expression=resource.name.startsWith(\"projects/_/buckets/${STATE_BUCKET}/objects/terraform/org/\"),title=read-upstream-org"
#   security → đọc org
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_SEC" --role="roles/storage.objectViewer" --condition="expression=resource.name.startsWith(\"projects/_/buckets/${STATE_BUCKET}/objects/terraform/org/\"),title=read-upstream-org"
#   workload → đọc org + connectivity
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_WL" --role="roles/storage.objectViewer" --condition="expression=resource.name.startsWith(\"projects/_/buckets/${STATE_BUCKET}/objects/terraform/org/\"),title=read-upstream-org"
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_WL" --role="roles/storage.objectViewer" --condition="expression=resource.name.startsWith(\"projects/_/buckets/${STATE_BUCKET}/objects/terraform/connectivity/\"),title=read-upstream-connectivity"
#   management → đọc org + connectivity + workload
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_MGMT" --role="roles/storage.objectViewer" --condition="expression=resource.name.startsWith(\"projects/_/buckets/${STATE_BUCKET}/objects/terraform/org/\"),title=read-upstream-org"
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_MGMT" --role="roles/storage.objectViewer" --condition="expression=resource.name.startsWith(\"projects/_/buckets/${STATE_BUCKET}/objects/terraform/connectivity/\"),title=read-upstream-connectivity"
gcloud storage buckets add-iam-policy-binding gs://$STATE_BUCKET --member="serviceAccount:$SA_MGMT" --role="roles/storage.objectViewer" --condition="expression=resource.name.startsWith(\"projects/_/buckets/${STATE_BUCKET}/objects/terraform/workload/\"),title=read-upstream-workload"
```

**Bước H — Cập nhật 5 tệp `backend.tf` với tên Bucket vừa tạo**

Mở từng tệp: [org/backend.tf](org/backend.tf), [connectivity/backend.tf](connectivity/backend.tf), [security/backend.tf](security/backend.tf), [workload/backend.tf](workload/backend.tf), [management/backend.tf](management/backend.tf) và thay giá trị bucket:

```hcl
terraform {
  backend "gcs" {
    bucket = "gcp-sg-tfstate-<UNIQUE_SUFFIX>"   # Thay bằng giá trị $STATE_BUCKET
    prefix = "terraform/<tên-stack>"
  }
}
```

**Bước I — Cập nhật `tf_runner_sa` trong 5 tệp `terraform.tfvars`**

Mặc định đã trỏ đúng tên SA: `sa-tf-{org,conn,sec,wl,mgmt}-001@gcp-platform-bootstrap-001.iam.gserviceaccount.com`. Nếu bạn đổi `SEED_PROJECT` khác, sửa lại field `tf_runner_sa` trong 5 tfvars cho khớp.

**Bước J — (CHẠY SAU `terraform apply org`) Hoàn tất role project-level cho sec/wl/mgmt**
...
gcloud projects add-iam-policy-binding $PRJ_SH_ACCESS --member="serviceAccount:$SA_WL" --role="roles/compute.instanceAdmin.v1" --condition=None
gcloud projects add-iam-policy-binding $PRJ_ASTRO     --member="serviceAccount:$SA_WL" --role="roles/compute.instanceAdmin.v1" --condition=None

# J3. sa-tf-mgmt-001 — Log Sinks/Buckets/Views + Monitoring + Budget
# projectIamAdmin cần thiết vì module log-export gán roles/logging.bucketWriter (project-level IAM)
# cho writer identity của log sink — thao tác này đòi resourcemanager.projects.setIamPolicy.
gcloud projects add-iam-policy-binding $PRJ_MGMT --member="serviceAccount:$SA_MGMT" --role="roles/logging.admin" --condition=None
gcloud projects add-iam-policy-binding $PRJ_MGMT --member="serviceAccount:$SA_MGMT" --role="roles/monitoring.admin" --condition=None
gcloud projects add-iam-policy-binding $PRJ_MGMT --member="serviceAccount:$SA_MGMT" --role="roles/storage.admin" --condition=None
gcloud projects add-iam-policy-binding $PRJ_MGMT --member="serviceAccount:$SA_MGMT" --role="roles/resourcemanager.projectIamAdmin" --condition=None
...
> 💡 **Vì sao tách Bước J ra riêng?** Một số role phải gán ở scope project, nhưng các project (`PRJ_MGMT`, `PRJ_SH_ACCESS`, ...) chỉ tồn tại SAU khi stack `org` chạy xong (do `random_string` sinh suffix). Vì vậy: chạy Bước 0→I trước → `terraform apply org` → quay lại chạy Bước J → tiếp tục các stack còn lại.

#### 🔨 Phase B — Tạo Runtime Service Account cho VM/Workload (sau khi apply stack `org`)

Các SA dưới đây không dùng để chạy Terraform mà dùng cho **runtime** của VM/workload (ví dụ: bastion host gắn `sa-sh-access`). Sau khi stack [org/](org/) apply thành công (các project app đã tồn tại), tiếp tục chạy trong cùng terminal (các biến `$PRJ_SH_ACCESS`, `$PRJ_ASTRO`, `$SA_WL` đã có sẵn từ Bước J — nếu mở shell mới, chạy lại Bước 0 và Bước J trước).
...
gcloud projects add-iam-policy-binding $PRJ_SH_ACCESS --member="serviceAccount:$BASTION_SA_EMAIL" --role="roles/monitoring.metricWriter"
gcloud projects add-iam-policy-binding $PRJ_SH_ACCESS --member="serviceAccount:$BASTION_SA_EMAIL" --role="roles/logging.logWriter"
...
gcloud projects add-iam-policy-binding $PRJ_ASTRO --member="serviceAccount:$ASTRO_SA_EMAIL" --role="roles/monitoring.metricWriter"
gcloud projects add-iam-policy-binding $PRJ_ASTRO --member="serviceAccount:$ASTRO_SA_EMAIL" --role="roles/logging.logWriter"
...
# Tools SA cho hub-net
export HUBNET_SA_EMAIL="gcp-sg-sa-hub-net-001@${PRJ_HUB_NET}.iam.gserviceaccount.com"
gcloud iam service-accounts create gcp-sg-sa-hub-net-001 --project=$PRJ_HUB_NET --display-name="hub-net runtime SA"
gcloud projects add-iam-policy-binding $PRJ_HUB_NET --member="serviceAccount:$HUBNET_SA_EMAIL" --role="roles/monitoring.metricWriter"
gcloud projects add-iam-policy-binding $PRJ_HUB_NET --member="serviceAccount:$HUBNET_SA_EMAIL" --role="roles/logging.logWriter"

# Tools SA cho sh-vpc
export SHVPC_SA_EMAIL="gcp-sg-sa-sh-vpc-001@${PRJ_SH_VPC}.iam.gserviceaccount.com"
gcloud iam service-accounts create gcp-sg-sa-sh-vpc-001 --project=$PRJ_SH_VPC --display-name="sh-vpc runtime SA"
gcloud projects add-iam-policy-binding $PRJ_SH_VPC --member="serviceAccount:$SHVPC_SA_EMAIL" --role="roles/monitoring.metricWriter"
gcloud projects add-iam-policy-binding $PRJ_SH_VPC --member="serviceAccount:$SHVPC_SA_EMAIL" --role="roles/logging.logWriter"
```
