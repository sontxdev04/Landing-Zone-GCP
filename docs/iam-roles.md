...
| 🟢 **`sa-tf-wl-001`** | [`workload/`](../workload/) | App Engineers | **Project (astronomy-shop):** `compute.instanceAdmin.v1` · `iam.serviceAccountUser` trên runtime SA |
...
| `astronomy-shop` | `sa-tf-wl-001` | `compute.instanceAdmin.v1` | Tạo VM workload |
...
Trên các project `astronomy-shop`, `hub-net`, `sh-vpc`, mỗi admin được cấp:
...
Admin được cấp `roles/logging.viewAccessor` **giới hạn** đúng vào log view `gcp-sg-logview-astronomy-shop-001` trong project `management` — chỉ đọc đúng logs của nguồn được phép, không thấy logs các project khác.
...
|:-----|:-----------|:--------|:-------:|:--------|
| `astro` | `gcp-sg-sa-astronomy-shop-001` | astronomy-shop | ✅ | Runtime SA cho workload Astronomy-shop |
