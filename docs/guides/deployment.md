...
| `01-bootstrap.sh` lỗi `storage.buckets.update access ... denied ... (or it may not exist)` ở Bước C (bật Object Versioning), **ngay sau khi** vừa tạo bucket | Race/propagation: bucket vừa `create` xong nhưng quyền/IAM trên bucket **chưa lan truyền** kịp khi lệnh `buckets update --versioning` chạy ngay sau đó. Không phải lỗi cấu hình | **Chạy lại `./scripts/01-bootstrap.sh`** (idempotent) — lệnh versioning sẽ thành công ở lần sau khi bucket đã propagate. Hoặc chạy riêng: `gcloud storage buckets update gs://$STATE_BUCKET --versioning` |
